...
HOUSEHOLDS_SHEET_NAME = "Households"
...
LOCAL_SCHOOL_OPTION = "Enrolling in local school for now"
...
def callback():
    ...
    if not id_token:
        return "Missing id_token from OAuth response.", 400
...
def get_logged_in_email():
    ...
    user = session["user"]
    return user.get("email") or user.get("upn") or user.get("preferred_username")
...
def get_form_data():
    ...
    try:
        sheet = get_sheet(worksheet_name=HOUSEHOLDS_SHEET_NAME)
    ...
    for idx, row in enumerate(records, start=2):
        ...
        if normalized_user_email in [p1_email, p2_email]:
            parent1 = (
                f'{row.get("Parent 1 Full Name", "")} '
                f'({row.get("PARENT 1: Email 1", "")})'
            ).strip()
            parent2 = (
                f'{row.get("Parent 2 Full Name", "")} '
                f'({row.get("PARENT 2: Email 1", "")})'
            ).strip()

            child_lines = []
            for i in range(1, 6):
                person_id = row.get(f"Student {i} \nPerson ID", "") or row.get(
                    f"Student {i}\nPerson ID", ""
                )
                full_name = row.get(f"Student {i} \nFull Name", "") or row.get(
                    f"Student {i}\nFull Name", ""
                )
                grade = row.get(f"Student {i} \nCurrent Grade", "") or row.get(
                    f"Student {i}\nCurrent Grade", ""
                )

                if full_name or person_id:
                    child_lines.append(f"{full_name} - {grade}")

            is_parent1 = normalized_user_email == p1_email
        ...
        updates.append(
            {
                "range": gspread.utils.rowcol_to_a1(
                    row_number, col_index("Return Date")
                ),
...
def get_location_form_data():
    ...
    try:
        households_sheet = get_sheet(HOUSEHOLDS_SHEET_NAME)
        household_records = households_sheet.get_all_records()

        location_sheet = get_sheet(LOCATION_SHEET_NAME)
            ...
            return jsonify(
                {
                    ...
                    "formData": {
                        ...
                        "Child1LearningMode": existing_location.get(
                            "Child1 Learning Mode", ""
                        ),
                        "Child2LearningMode": existing_location.get(
                            "Child 2 Learning Mode", ""
                        ),
                        "Child3LearningMode": existing_location.get(
                            "Child 3 Learning Mode", ""
                        ),
                        "Child4LearningMode": existing_location.get(
                            "Child 4 Learning Mode", ""
                        ),
                        "Child5LearningMode": existing_location.get(
                            "Child 5 Learning Mode", ""
                        ),
                        "Child1School": existing_location.get(
                            "School your child 1 will be attending", ""
                        ),
                        "Child2School": existing_location.get(
                            "School your child 2 will be attending", ""
                        ),
                        "Child3School": existing_location.get(
                            "School your child 3 will be attending", ""
                        ),
                        "Child4School": existing_location.get(
                            "School your child 4 will be attending", ""
                        ),
                        "Child5School": existing_location.get(
...
def location_notice():
    ...
    # Clear school values if learning mode is not local school
    for i in range(1, 6):
        learning_mode = data.get(f"Child{i}LearningMode", "")
        if learning_mode != LOCAL_SCHOOL_OPTION:
            data[f"Child{i}School"] = ""
    ...
    def get_value(header_name):
        mapping = {
            ...
            "School your child 1 will be attending": data.get("Child1School", ""),
            "School your child 2 will be attending": data.get("Child2School", ""),
            "School your child 3 will be attending": data.get("Child3School", ""),
            "School your child 4 will be attending": data.get("Child4School", ""),
            "School your child 5 will be attending": data.get("Child5School", ""),
        }
...
#             {
#                 "range": gspread.utils.rowcol_to_a1(
#                     row_number, col_index("Updated By")
#                 ),
#                 "values": [[data.get("Who_is_completing_the_form", "")]],
#             }
#         )
...
#             {
#                 "range": gspread.utils.rowcol_to_a1(
#                     row_number, col_index("Updated On")
#                 ),
#                 "values": [[timestamp]],
#             }
#         )
